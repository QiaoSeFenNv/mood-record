...
- [ ] 使用 imagegen skill 生成一棵基础树和五类透明原创效果层，记录生成/后处理清单；每类关键动画控制在 6 到 8 帧。
- [ ] 实现 DayTimeline 和记录详情删除确认。
- [ ] 松手使用一次 clientMutationId 保存；成功后显示 5 秒 UndoBar，失败保留选值并复用幂等键重试。
...
- 动画素材：先完成静态关键帧和低动效路径，再启用逐帧资源；出现性能问题可配置回退而不影响数据。
